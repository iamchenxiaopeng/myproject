<template>
  <div class="index">
    <div class="editor" id="editor" v-show="showEditor">
      ...
      <div class="boxtool" @click="clearselect">
      ...
      <div class="container">
        <div id="mycanvas" class="mycanvas" :style="{height: canvasHeight}">
          ...
          <div class="layerbox cannotselect" @mousedown.stop="startMove($event)" @mouseup="endMove($event)"
            v-for="(layerBox, index) in layerBoxs" :key="index"
    ...
    <div class="choosematerial" v-show="showMaterial">
      ...
      <div class="boxtool">
        ...
        <div class="boxtool-search" style="text-align:center;line-height: 30px">
          <!-- <input class="mysearch" type="text" placeholder="搜索图层名字或者ID">
          <div class="searchicon"><img src="../../assets/edit/search.png" alt=""></div> -->
          <span style="font-size: 16px; color: red;font-weight: bold;margin-left: 50px;">我要上传自己的图片：</span>
          <input id="mypicfile" @change="add_img" type="file">
          <button style="width: 180px;height: 30px;cursor: pointer">确定上传</button>
        </div>
      </div>
      <div class="choosees">
        <div class="animatekind">
          <div :class="{animatekindactive: index == 0}" class="animatekind-list-m cannotselect"
            v-for="(material, index) in materialList" :key="index" @click="selectMaterial(material.id,index)">{{material}}
          </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        singleMaterials: [{
            ...
            {
              'title': '图片',
              'iconUrl': require('../../assets/img/7.png'),
              'width': 800,
              'height': 520
            },
            {
              'title': '图片',
              'iconUrl': require('../../assets/img/8.png'),
              'width': 800,
              'height': 800
            },
            {
              'title': '图片',
              'iconUrl': require('../../assets/img/9.png'),
              'width': 188,
              'height': 100
            },
            {
              'title': '图片',
              'iconUrl': require('../../assets/img/10.png'),
              'width': 930,
              'height': 600
            },
          ], //某种素材
    ...
    methods: {
      init() {},
      getOnlinePic(){
        this.$axios.get("./../web/getpic").then((res)=>{
          this.singleMaterials = this.createImgDate(res.data.data)
      ...
      add_img() {
        ...
        var img1 = document.querySelector('#mypicfile').files[0];
        if(!this.judgeType(img1.name)){
          return
        }
				var uri = ''
        ...
        form.append('file', img1, img1.name);
				//接口部分
				this.$axios.post("./../web/uploadpic", form, {
					headers: {
						'Content-Type': 'multipart/form-data'
					}
				}).then(response => {
					alert('上传完成！您可以使用自己的图片了')
          this.getOnlinePic()
				}).catch(function(err) {
					console.log(err);
				});
      ...
      getData() {
        ...
        //     console.log(err);
        //   });
        this.materialList = ['本地图片','用户上传的图片'];
        this.selectMaterial(1,0);
      },
      ...
      saveEdit() {
        ...
        html2canvas(document.querySelector("#mycanvas"), {
          ...
          _this.downLoadQRcode(canvas.toDataURL())
        });
      },
      //打开选择动画页面
      openAnimation() {
      ...
      downLoadQRcode(imgData) {
        this.downloadFile('helloworld.jpg', imgData);
      },
      // 下载
